...
for i in inputs:
    ...
    if i[1]=="stop":
        ...
        frame_num += 1
        #------------------detection begins--------------------------------------------------------  
        if True : #VOTE_FRAME = 3, then 0,1,2 allowed
        #     #------------------reset------------------------    
        ...
        axarr[0, 0].imshow(cv2.cvtColor(img3f, cv2.COLOR_BGR2RGB), cmap=plt.gray())
        axarr[0, 0].set_title('Original Image')

        axarr[0, 1].imshow(img1f, cmap=plt.gray())
        axarr[0, 1].set_title('Heatmap')
        axarr[1, 0].imshow(img2f, cmap=plt.gray())
        axarr[1, 0].set_title('Red Thresholding')
        axarr[1, 1].imshow(cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB), cmap=plt.gray())
        axarr[1, 1].set_title('Final Annotations')
        plt.pause(0.0001)
        print frame, " took ",  time.time()-t, "s"
        frame+=1
